...
<script language="javascript">
	...
	function validaSelecaoItem() {
		...
		var retorna  = true;
			...
			if ( document.forms[0].oidItemLancamento.selectedIndex == -1 ) {
				mensagem += '\n - Você deve selecionar um item do lançamento';
				retorna = false; }
	...
	function validaExclusaoLanc() {
		...
		var retorna  = true;
			...
			if( !confirm( 'Tem certeza que deseja excluir lançamento?' ) )
					  retorna = false;
	...
	function validaDados( operacao ) {
		...
		var retorna  = true;
		  ...
		  if ( document.forms[0].valor.value <= 0 ) {
			mensagem += '\n - Valor inválido (não pode ser negativo ou zero)...';
			retorna = false; }
	...
	function validaCodigoLanc() {
		...
		var retorna  = true;
		  ...
		  if ( testaNulo( document.forms[0].oidLancamento ) ) {
			mensagem += '\n - Código do lançamento não preenchido...';
			retorna = false; }
	...
	function validaInclusaoLanc() {
		...
		var retorna  = true;
		  ...
		  if ( testaNulo( document.forms[0].dataLanc ) ) {
			mensagem += '\n - Data do lançamento não preenchida';
			retorna = false; }
	...
	//--------------------------------------------
	// validaConta()
	// - Testa validade da conta contábil
	//--------------------------------------------
	function validaConta() {
		...
		if ( !retorna && !testaNulo( document.forms[0].oidPlano ) ) {
			alert( 'Conta não cadastrada... Consulte a lista' );
				document.forms[0].oidPlano.focus(); }
	...
	function validaHistorico() {
		...
		if ( !retorna ) {
			alert( 'Histórico padrão não cadastrado... Consulte a lista' );
	...
	// - Redireciona para a tela de inclusão de itens
	//--------------------------------------------
...
<body class="pagina">
	...
	<div align="center">
		...
		case 7: { // Alteração de lancamentos...
	...
	break; }
		 ...
		 // Inclui item de lancamento (na alteração)...
