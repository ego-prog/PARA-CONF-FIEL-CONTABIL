	...
	function validaSelecaoItem(opcao) {
		...
		var retorna  = true;
			...
			if ( document.forms[0].oidCentroCusto.selectedIndex == -1 ) {
				mensagem += '\n - Você deve selecionar um centro de custo';
				retorna = false; }
...
?>
		...
		case 3: { // Consulta de Centros de Custo...
			...
			"select centro.codigo as \"Código\", emp.razaosocial as \"Empresa\", centro.sigla as \"Sigla\", centro.descricao as \"Descrição\" from centrocusto_cont centro, empresa_cont emp where codigoempresa = emp.codigo and codigoempresa = '$oidEmpresaCont' and emp.codigocliente = '$oidEmpresa' order by centro.codigo, centro.descricao;",
										false );
			break; }

		case 4: { // Alteração de Centro de Custo...
