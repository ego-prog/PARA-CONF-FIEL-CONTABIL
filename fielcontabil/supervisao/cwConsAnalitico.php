...
<!--
	...
	Módulo: cwConsAnalitico.php
	  Consulta Balancete Analítico
-->
<html>
<head>
...
<?PHP
	...
	if ( empty( $controleNavegacao ) )
	 ...
	 default: {
		  ...
		  $cabec = new TituloCw( $cabecConsAnalitico );
		  $cabec->mostra();
?>
<body class="pagina" onLoad="this.document.formConsAnalitico.dataInicial.focus();">

	<div align="center">

	<br><br>

	<form action="cwConsAnalitico.php"
				name="formConsAnalitico" method="get" onSubmit="return validaDados();">
	...
	<input type="hidden" name="controleNavegacao" value="1">
		...
		<table border="0" width="100%">
			...
			<td background="../imagens/cw_janela.gif" border="0" width="85%" class="tjanela">
				<?PHP echo $tituloConsAnalitico; ?>
			</td>
			<td width="15%" align="center" class="tjanela">
				<input type="button" class="btitulo" name= "bt_ajuda"
					value="<?= $botaoAjudaJanela; ?>" onClick="javascript:abreAjuda('../cw_ajuda.html#cwConsAnalitico')">
		...
		<table class="cjanela" border="0" width="100%">
			...
			<tr>
				<td width="30%" align="right" class="cjanela">
				  <?PHP echo $campoNivelMaximoExibir; ?>
				</td>

				<td width="70%" align="left" class="cjanela">
					<select name="nivelConta" class="dljanela">
					   <option value="1"><?= $campoNivel1; ?></option>
					   <option value="2"><?= $campoNivel2; ?></option>
					   <option value="3"><?= $campoNivel3; ?></option>
					   <option value="4"><?= $campoNivel4; ?></option>
					   <option value="5" selected><?= $campoNivel5; ?></option>
					   <option value="6"><?= $campoNivel6; ?></option>
					</select>
				</td>
			</tr>
			...
			<tr>
				...
				<td width="70%" align="left">
					...
					OnClick="javascript:abreAjuda('../cw_ajuda.html#cwConsAnalitico');">
				</td>
			</tr>
...
<?PHP
	 ...
	 case 1: {
		 ...
		 if ( $lancamento->consultaBalanceteAnalitico( $dataInicial, $dataFinal, 
					$oidEmpresaCont, $oidEmpresa, $paginaInicial, 
					$exibeNaoLiberado, $exibeContador,
				$perfilUsuario, $nivelConta, $desconsiderarZeramento ) == false ) {
...
<?
		 ...
		 $lancamento = new Lancamento();
		 $lancamento->consultaBalanceteAnaliticoPDF( $dataInicial, $dataFinal,
				$oidEmpresaCont,
				$oidEmpresa, $paginaInicial, $exibeContador, $nivelConta, $desconsiderarZeramento );
