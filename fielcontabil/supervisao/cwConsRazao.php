<?PHP
	...
	$conta = new Conta();
	$listaContas = $conta->buscaConta( 0, "", 6 );
...
<script language="javascript">
	...
	function carregaColecao() {
		...
		echo "\n	    // ".$listaEmpresas[$indx][1]."\n	    newEmpresaConta();\n";
		     // Laco de contas...
             echo "	    newConta( '', '0', '".$msgTodasContas."', '".$listaContas[0][3]."' );\n";
...
<?PHP
	...
	if ( empty( $controleNavegacao ) )
	 ...
	 default: {
		  ...
		  $cabec = new TituloCw( $cabecConsRazao );
		  $cabec->mostra();
?>
<body class="pagina" onLoad="carregaColecao();this.document.formConsRazao.dataInicial.focus();">

	<div align="center">

	<br><br>

	<form action="cwConsRazao.php"
				name="formConsRazao" method="get" onSubmit="return validaDados();">
	...
	<input type="hidden" name="controleNavegacao" value="1">
		...
		<table border="0" width="100%">
			...
			<td background="../imagens/cw_janela.gif" border="0" width="85%" class="tjanela">
				<?PHP echo $tituloConsRazao; ?>
			</td>
			<td width="15%" align="center" class="tjanela">
				<input type="button" class="btitulo" name= "bt_ajuda"
					value="<?= $botaoAjudaJanela; ?>" onClick="javascript:abreAjuda('../cw_ajuda.html#cwConsRazao')">
		...
		<table class="cjanela" border="0" width="100%">
			...
			<tr>
				...
				<td width="70%" align="left">
					...
					OnClick="javascript:abreAjuda('../cw_ajuda.html#cwConsRazao');">
				</td>
			</tr>
...
<?PHP
	 ...
	 case 1: {
		 ...
		 if ( $lancamento->consultaRazaoAnalitico( $dataInicial, $dataFinal,
					$oidEmpresaCont, $codigoSintetico, $oidEmpresa, $paginaInicial, $exibeNaoLiberado,
...
<?
		 ...
		 $lancamento = new Lancamento();
		 $lancamento->consultaRazaoAnaliticoPDF( $dataInicial, $dataFinal,
							$oidEmpresaCont, $oidConta, $oidEmpresa, $paginaInicial,
							$exibeContador );
		break;
?>
	</div>
