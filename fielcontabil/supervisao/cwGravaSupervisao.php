	...
	switch( $tipoOperacao ) {
		...
		case 1: {
			...
			$empresa->setEmpresa( $oidEmpresa, $razaoSocial,
					      ...
					      $responsavel, $cpfResponsavel, $mascaraPlano, $mascaraDoar, $codigoCaixa );
			...
			for ( $indx = 0; $indx < sizeof( $listaLancamentos ); $indx++ ) {
				...
				if ( $listaLancamentos[$indx][1] > 0 ) {
					...
					$resposta = $lancamento->incluiItemLancamento( $lancamento->getOidLancamento(), 
										$zeramento->getContrapartida(), str_replace("[nomeconta]", $contrapartida->getDescricao(), $historicoDestino), $listaLancamentos[$indx][1],
										"D", $oidZeramento, 0, "" );				
				...
				if ( $listaLancamentos[$indx][1] < 0 ) {
					...
					$contrapartida->pesquisaContaSemDV( $listaLancamentos[$indx][0]);
					$resposta = $lancamento->incluiItemLancamento( $lancamento->getOidLancamento(), 
						$zeramento->getContrapartida(), str_replace("[nomeconta]", $contrapartida->getDescricao(), $historicoDestino), $saldo,
						"C", $oidZeramento,0, "" );
		...
		case 28: {
			...
			$centroCusto->setCentroCusto( $oidEmpresaCont, $sigla, $descricao );

			// Verifica se gravou
			if ( $centroCusto->grava() )
				$msg = new MsgCw( $msgIncluiCentroOk );
			else
				$msg = new MsgCw( $msgIncluiCentro );

			// Exibe mensagem...
			$msg->mostra();

			// seta atributos para LOG
			$descricaoLog = $msgLogIncluiCentro;
			$complementoLog = $oidEmpresa." ".$sigla . " " . $descricao;

			break; }

		// Alteracao de Centro de Custo...
		case 29: {

			// Instancia classe
			$centroCusto = new CentroCusto();

			// seta variaveis de classe...
			$centroCusto->setCentroCusto( $oidEmpresaCont, $sigla, $descricao );

			$centroCusto->setOidCentroCusto( $oidCentroCusto );

			// Verifica se gravou
			if ( $centroCusto->grava(false) )
				$msg = new MsgCw( $msgAlteraCentroOk,
					"../imagens/contabil.jpg", "javascript:history.go(-3);" );
			else
				$msg = new MsgCw( $msgAlteraCentro,
					"../imagens/contabil.jpg", "javascript:history.go(-3);" );
			...
			// seta atributos para LOG
			$descricaoLog = $msgLogAlteraCentro;
			$complementoLog = $oidEmpresa." ".$oidCentroCusto;

			break; }

		// Exclusao de Centro de Custo...
		case 30: {
			...
			// Verifica se gravou
			if ( $centroCusto->exclui() )
				$msg = new MsgCw( $msgExcluiCentroOk,
					"../imagens/contabil.jpg", "javascript:history.go(-2);" );
			else
				$msg = new MsgCw( $msgExcluiCentro,
					"../imagens/contabil.jpg", "javascript:history.go(-2);" );
			...
			// seta atributos para LOG
			$descricaoLog = $msgLogExcluiCentro;
			$complementoLog = $oidEmpresa." ".$oidCentroCusto;
